...
STATE_TO_EVALUATE = 1000
EVAL_EVERY_FRAME = 100

class DistributionalDQN(nn.Module):
	"""docstring for DistributionalDQN"""
	def __init__(self, input_shape, n_actions):
		super(DistributionalDQN, self).__init__()
		self.conv = nn.Sequential(
			nn.Conv2d(input_shape[0], 32, kernel_size= 8, stride= 8),
			nn.ReLU(),
...
if __name__ == '__main__':
	...
	writer = SummaryWriter("../../logs/07/07_dqn_distrib", comment= "-" + params['run_name'] + "-basic")
	...
	with common.RewardTracker(writer, params['stop_reward']) as reward_tracker:
		while True:
			...
			if eval_states is None:
				eval_states = buffer.sample(STATE_TO_EVALUATE)
