...
def calc_loss_dqn(batch, net, tgt_net, gamma, device= "cpu", double= True):
    ...
    if double:
        next_state_actions = net(states_v).max(1)[1]
...
class RewardTracker:
    ...
    def reward(self, reward, frame, epsilon= None):
        ...
        print("%d: done %d games, mean reward %.3f, speed %.2d f/s%s" % (
...
def distr_projection(next_distr, rewards, dones, Vmin, Vmax, n_atoms, gamma):
    """
    Perform distribution projection aka Categorical Algorithm form the
